...
def get_new_predicted_revenue(session: Session, new_allocations:dict, season=str) -> float:
        return (
            session
                .sql(f"SELECT predict_roi_mfc(array_construct({season},{new_allocations['search_engine']}, {new_allocations['social_media']}, {new_allocations['video']}, {new_allocations['email']}))")
                .to_pandas()
                .iloc[0,0]
        )
